"""Adapter tests — how Pilot consumes a manifest-provided action through the
`ActionExecutor` seam. When `sconixcore` ships its manifest executor, `EXECUTOR`
...
from pilot.executor import DEFAULT, ExecResult, LocalExecutor
...
def _spec(name: str, *, approval: ApprovalMode, risk: Risk = Risk.EXTERNAL_WRITE) -> ActionSpec:
    return ActionSpec(
        name=name,
        argv=("sx", name, "<app>"),
        risk=risk,
...
class FakeExecutor:
    """Stands in for the future sconixcore manifest executor."""
    ...
    async def execute(self, name: str, *, target: str, **args: object) -> ExecResult:
...
def test_lookup_by_name() -> None:
    ex = LocalExecutor()
    assert ex.lookup("restart_app") is act.RESTART
    assert ex.lookup("deploy") is None
...
async def test_execute_binds_target_into_argv_without_a_shell(tmp_path) -> None:
    ...
    act.TARGETS_PATH = tf
    try:
        res = await LocalExecutor().execute("restart_app", target="drill")
...
async def test_execute_undeclared_action_raises() -> None:
    with pytest.raises(KeyError):
        await LocalExecutor().execute("delete_env", target="drill")
...
async def _decide(session, executor, *, principal=PILOT, allow=None, tool="restart_app"):
    rr: dict[str, str] = {}
...
async def test_gate_reads_approval_never_and_allows(session) -> None:
    ex = FakeExecutor({"restart_app": _spec("restart_app", approval=ApprovalMode.NEVER)})
    out, rr, calls = await _decide(session, ex, allow=set())  # not in allow-set…
...
async def test_gate_reads_approval_always_and_denies(session) -> None:
    ex = FakeExecutor({"restart_app": _spec("restart_app", approval=ApprovalMode.ALWAYS)})
    out, rr, calls = await _decide(session, ex)
...
async def test_gate_enforces_principal_scope(session) -> None:
    ...
    ex = FakeExecutor({"restart_app": _spec("restart_app", approval=ApprovalMode.POLICY)})
    out, rr, calls = await _decide(session, ex, principal=scoped)
    assert "scope does not include drill" in out and calls == []


async def test_executor_result_reaches_the_audit_row(session) -> None:
    ex = FakeExecutor(
        {"restart_app": _spec("restart_app", approval=ApprovalMode.POLICY)},
    ...
    async def _fn(app: str) -> str:
        ...
        res = await ex.execute("restart_app", target=app)
        await record(
            session,
            target=app,
            tool="restart_app",
